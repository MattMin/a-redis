...
这套环境的集群节点在 Docker 内部通过 `host.docker.internal` + 宿主机映射端口互相通信，A-Redis 可以直接使用宿主机地址 `127.0.0.1:7001` 接入并自动发现其它节点。
...
> 说明：节点会直接宣告宿主机可访问的 `host.docker.internal:700x` / `1700x` 端点；A-Redis 若从 `127.0.0.1:7001` 接入，会按同端口自动发现其它节点。
